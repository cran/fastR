...
vec logistic_only(mat x, colvec y){
  ...
  vec p(n),L(pcols);
  colvec b_old=Rcpp::runif(d,-1,1),b_new(d),L1(d),yhat(n),L2_L1(d);
  double dif,s,t,sw=0.0,szw=0.0,sz2w=0.0;
  for(j=0;j<n;j++){
    z(j,0)=1;
  }
  for(i=0;i<pcols;i++){
...
//[[Rcpp::export]]
vector<int> nth(vector<int> x,unsigned int elem){
  nth_element(x.begin(),x.begin()+elem-1,x.end());
  return x;
}

//[[Rcpp::export]]
rowvec colmeds(mat x){
  ...
  if(sz%2==0)
    for(i=0;i<p;i++){
      ...
      nth_element(x.begin_col(i)+middle+1,x.begin_col(i)+middle+1,x.end_col(i));
      F(i)=(x(middle,i)+x(middle+1,i))/2.0;
    }
  else 
    for(i=0;i<p;i++){
